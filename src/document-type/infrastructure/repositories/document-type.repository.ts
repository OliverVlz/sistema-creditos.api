...
type CreateDocumentTypeData = Omit<Partial<DocumentType>, 'id' | 'createdAt' | 'updatedAt'> & {
  code: string;
  name: string;
  createdBy: string;
};

type UpdateDocumentTypeData = Partial<Omit<DocumentType, 'id' | 'createdAt' | 'updatedAt' | 'createdBy'>> & {
  updatedBy?: string;
};
...
export class DocumentTypeRepository {
  ...
  async createDocumentType(data: CreateDocumentTypeData): Promise<DocumentType> {
  ...
  async updateDocumentType(id: string, updateData: UpdateDocumentTypeData): Promise<DocumentType> {
  ...
  async searchDocumentTypesWithPagination(searchData: DocumentTypeSearchData) {
    const queryBuilder = this.documentTypesRepository.createQueryBuilder('documentType')
      .leftJoinAndSelect('documentType.creator', 'creator');
    ...
    if (searchData.isActive !== undefined) {
      queryBuilder.andWhere('documentType.isActive = :isActive', { isActive: searchData.isActive });
    }

    queryBuilder.orderBy('documentType.displayOrder', 'ASC')
